...
<section class="content">
    ...
    <div class="row">
        ...
        <div class="col-lg-3">
            <div>
                <button id="cansel_payment" class="btn btn-flat btn-warning">
...
$(document).on('click', '#del_cart', function() {
    if(confirm('Apakah Anda yakin Lillahitaala?')) {
...
$(document).on('click', '#edit_cart', function() {
    ...
    if(price == '' || price < 1) {
        alert('Piye to bro!? Hargane ora oleh kosong bro!, niat dodolan opo ora?')
        $('#price_item').focus()
    } else if(qty == '' || qty < 1) {
        alert('Piye to bro!? Qty ora oleh kosong bro!')
        $('#qty_item').focus()
    } else if(discount == '') {
        $('#discount_item').val(0)
    } else {
        $.ajax({
            ...
            success: function(result) {
                if(result.success == true) {
                    ...
                    alert('Alhamdulillah!! Item cart berhasil ter-update Bro!')
                    $('#modal-item-edit').modal('hide');
                } else {
                    alert('Astagfirullah!! Gagal ter-update bro!');
                }
            }
        })
    }
...
$(document).on('click', '#cancel_payment', function() {
    if(confirm('Opo panjenengan yakin bro?')) {
        $.ajax({
            type: 'POST',
            url: '<?=site_url('sale/cart_del')?>',
            dataType: 'json',
            data: {'cancel_paymen': true},
            success: function(result) {
                if(result.success == true) {
                    $('#cart_table').load('<?=site_url('sale/cart_data')?>', function() {
                        calculate()
